...
const UserAvatar = ({
  ...
  const getAllServicesAndRoles = () => {
    ...
    return [{ service: "", role: user.user_role, color: "blue" }];
  };
    ...
    <>
      <Dropdown placement="bottom-end">
        ...
        <DropdownMenu aria-label="Profile Actions" variant="flat">
          <DropdownItem
            key="legal_name"
            className="text-postDark dark:text-postLight"
            textValue="User"
          >
              ...
              <div className="flex flex-col gap-2 mt-2 max-w-60">
                {servicesAndRoles.map((item, index) => (
                  <div key={index} className="flex items-center gap-2">
                    <div
                      className={`flex items-center gap-1 px-2 py-1 rounded-full ${item.color === "blue"
                    ...
                    >
                      ...
                      ></div>
                      <span
                        className={`text-xs font-medium ${item.color === "blue"
          ...
          <DropdownItem
            key="Language"
            className="px-2 gap-2 text-postDark dark:text-postLight"
            textValue="Language"
          >
            {/* <p className='text-black text-lg dark:text-white text-center'>{lang == "bn" ? "ভাষা" : "Language"}</p> */}
            <div className="px-2">
              <LanguageTogglerMenu lang={lang} />
            </div>
          ...
          <DropdownItem
            key="logout"
            className="gap-2 font-semibold text-danger dark:text-postLight"
            color="danger"
            textValue="Log Out"
          >
            <Button
