...
export interface RMSBranchSearchInputProps {
...
export default function RMSBranchSearchInput({
    selectedBranch,
    label,
    placeholder,
    isRequired,
    ...
    const [searchBranchQueryParams, setSearchBranchQueryParams] = useState<SearchBranchQuery>({
        ...
        page: 1,
        per_page: 100,
        status: "EDBO,EDSO,GPO,HO,MAIN,SO,TSO,UPO,MSS,RMS"
    });
        ...
        >
            {(item: Branch) => (
                <AutocompleteItem key={item.branch_code} textValue={item.name}>
                    <p className="text-postDark dark:text-postLight">{item.name}, {item.circle}, {item.upzilla}, {item.district}</p>
                </AutocompleteItem>
            )}
        </Autocomplete>
    );
