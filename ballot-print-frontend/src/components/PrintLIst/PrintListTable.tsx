...
import { PrintedListLocale } from "@/dictionaries/types";
...
const PrintedListTable = ({
  lang,
  printedListLocale,
}: {
  lang: Locale;
  printedListLocale: PrintedListLocale;
}) => {
      ...
      >
        ...
        <div className="w-full md:w-[49%] lg:w-[49%] xl:w-[49%] 2xl:w-[49%]">
          <Select
            ...
            variant="bordered"
            label={printedListLocale.select_a_period}
            labelPlacement="outside"
            placeholder={printedListLocale.select_a_period}
          ...
          >
            {initialDateTimeRangeOptionsBn.map((option) => (
              <SelectItem key={option.key}>
                {
                  printedListLocale[
                    option.nameKey as keyof typeof printedListLocale
                  ]
                }
              </SelectItem>
            ))}
        ...
        <div className="w-full md:w-[49%] lg:w-[49%] xl:w-[49%] 2xl:w-[49%]">
          <DateRangePicker
            ...
            label={printedListLocale.select_date_range}
            labelPlacement="outside"
        ...
        <div className="flex justify-between items-center mb-4">
          <div className="w-full md:w-[49%]">
            <Input
              size="md"
              aria-label="Search"
              label={printedListLocale.search}
              labelPlacement="outside"
              placeholder={
                printedListLocale.search_by_serial_no_outbound_or_inbound_address
              }
              variant="bordered"
        ...
        <div className="w-full grid grid-cols-1 ">
          <div className="flex justify-between items-center mx-2 my-2">
            <span className="text-default-400 text-small">
              {printedListLocale.total} :{" "}
              {lang === "bn"
                ? e_to_b(filteredBallotRecords.length.toString())
                : filteredBallotRecords.length}{" "}
              {printedListLocale.ballot_records}
            </span>

            <div className="flex justify-center items-center">
              <label className="flex items-center text-default-400 text-small">
                {printedListLocale.rows_per_page} :{" "}
                <select
            ...
            >
              <TableHeader columns={columns}>
                {(column) => {
                  const tableHeader =
                    printedListLocale[
                      column.name as keyof typeof printedListLocale
                    ] || column.name;
                ...
                emptyContent={
                  <div className="text-center py-8">
                    <p className="ss:text-sm xxs:text-sm xs:text-base sm:text-base">
                      {printedListLocale.no_ballot_data_found}
                    </p>
                  </div>
                }
              >
...
export default PrintedListTable;
