...
import PostSenderNewAddressModal from "./PostSenderNewAddressModal";
import PostSenderEditAddressModal from "./PostSenderEditAddressModal";

export interface SenderAddressHistoryModalProps {
  post: Post;
...
export default function SenderAddressHistoryModal({
  initialPersonType,
  post,
  person_to_set,
  addressLocale,
...
}: SenderAddressHistoryModalProps) {
  ...
  const [editingPerson, setEditingPerson] = useState<Person>(INITIAL_SENDER);
  const [selectedPerson, setSelectedPerson] = useState<Person>(INITIAL_SENDER);
                  ...
                  <div className="flex w-full flex-col">
                    ...
                    <div>
                      <div className="ss:p-1 sm:p-2 lg:p-3 grid xxs:grid-cols-1 xs:grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div
                          onClick={() =>
                            handleOpenAddAddress(PersonType["SENDER"])
                          }
                        ...
                        {persons && persons
                          .filter(
                            (person) =>
                              person.person_type == PersonType["SENDER"]
                          )
