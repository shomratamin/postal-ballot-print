...
export interface PostRecipientEditAddressModalProps {
  isOpen: any;
...
export default function PostRecipientEditAddressModal({ isOpen, onClose, lang, editingPerson, post, countries, countryMap, addressLocale, validationLocale, commonLocale, addressType }: PostRecipientEditAddressModalProps) {
  ...
  const isInternational = post?.type == PostType["INTERNATIONAL"] || editPerson.address_type == AddressType["INTERNATIONAL"] || addressType == AddressType["INTERNATIONAL"];
  const isPostDefinedAndInternational = post && post.type == PostType["INTERNATIONAL"];
  const isDomestic = post?.type == PostType["DOMESTIC"] || editPerson.address_type == AddressType["DOMESTIC"] || addressType == AddressType["DOMESTIC"];
            ...
            <>
              <form onSubmit={formik.handleSubmit}>
                ...
                <ModalBody>
                  ...
                  {((post && post.type == PostType["INTERNATIONAL"]) || editPerson.address_type == AddressType["INTERNATIONAL"]) ? (
                    <motion.div
                      className="pb-6"
                      key="country"
                      // transition={{ ease: "easeInOut" }}
