...
import NewAddressModal from "./PostRecipientNewAddressModal";
import EditAddressModal from "./PostRecipientEditAddressModal";
import {
  Country,
  District,
  PostOffice,
...
export default function AddressHistoryModal({
  ...
  const [selectedTab, setSelectedTab] = React.useState<string | number>(
    activePersonType == PersonType["RECIPIENT"] ? "recipient" : "sender"
  );
  ...
  const changePersonType = (key: string | number) => {
    setSelectedTab(key);
    if (key == "recipient") {
      updateActivePersonType(PersonType["RECIPIENT"]);
    } else if (key == "sender") {
    ...
    //   return persons.filter((per) => per.person_type == person_type)
    //     .length;
                  ...
                  <div className="flex w-full flex-col">
                    <Tabs
                      ...
                      onSelectionChange={(key: string | number) =>
                        changePersonType(key)
                      }
                    >
                      <Tab
                        key="recipient"
                        title={addressLocale.recipient_addresses}
                      >
                        <div>
                      ...
                      <Tab key="sender" title={addressLocale.sender_addresses}>
                        <div>
                          <div className="ss:p-1 sm:p-2 lg:p-3 grid xxs:grid-cols-1 xs:grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div
                              onClick={() =>
