...
export default function PostRecipientNewAddressModal({ isOpen, onClose, lang, addressLocale, commonLocale, validationLocale, post, personType, countries, countryMap, addressType }: PostRecipientNewAddressModalProps) {
  ...
  const formik = useFormik({
      ...
      phone: Yup.string()
        .test('phone-length', function (value) {
          ...
          if (value && value.length) {
            const countryCode = countryMap[`${post?.country_id}`].code; // Assume `countryCode` exists in form values
    ...
    onSubmit: async (values) => {
      ...
      let person_request: PersonStoreRequest = {
        ...
        country: isPostDefinedAndInternational ? post.country : person.country,
        country_id: isPostDefinedAndInternational ? post.country_id : person.country_id,
        zone: isPostDefinedAndInternational ? post.zone : person.zone,
        zone_id: isPostDefinedAndInternational ? post.zone_id : person.zone_id,
  ...
  const isInternational = post?.type == PostType["INTERNATIONAL"] || person.address_type == AddressType["INTERNATIONAL"] || addressType == AddressType["INTERNATIONAL"];
  const isPostDefinedAndInternational = post && post.type == PostType["INTERNATIONAL"];
  const isDomestic = post?.type == PostType["DOMESTIC"] || person.address_type == AddressType["DOMESTIC"] || addressType == AddressType["DOMESTIC"];
                    ...
                    {isInternational ? (
                      ...
                      >
                        <MaterialSelect
                          ...
                          isDisabled={false}
                          isRequired={false}
                          whenChange={isPostDefinedAndInternational ? () => false : handleCountrySelectionChange}
                          items={
                            contrySelectionRows
                          }
                          value={isPostDefinedAndInternational ? `${post.country}` : (person.country?.length !== 0 ? `${person.country}` : "")}
                        />
                      </motion.div>
                    ) : (
                      isDomestic && (<>
                        ...
                        >
                          <MaterialSelect
                            lang={lang}
                            label={addressLocale.subdistrict_thana}
                            id="police_station"
                            name="police_station"
                        ...
                        <div className="flex w-full justify-between">
                          <div className="w-[18%]">
                            <MaterialInput
                              ...
                              whenChange={() => { }}
                              whenBlur={() => { }}
                              value={`+${countryPhoneCodeMap[countryMap[`${post?.country_id}`].code].phone_code} `}
                              error={""}
                              type="text"
