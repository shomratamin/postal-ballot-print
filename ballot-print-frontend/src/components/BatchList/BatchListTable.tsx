...
import { BatchListLocale} from "@/dictionaries/types";
...
const BatchListTable = ({
  ...
  lang: Locale;
  batchListLocale:BatchListLocale;
}) => {
  ...
  const [filteredBallotRecords, setFilteredBallotRecords] = useState(ballotData);
  ...
  const currentRecords = filteredBallotRecords.slice(indexOfFirstRecord, indexOfLastRecord);
  const totalPages = Math.ceil(filteredBallotRecords.length / per_page);

  // Handle ballot action
  const handleBallotAction = (ballot: typeof ballotData[0], actionType: string) => {
    ...
    if (searchValue) {
      const filtered = ballotData.filter(
        (item) =>
          ...
          item.outbound_address.toLowerCase().includes(searchValue.toLowerCase()) ||
       ...
       >
         ...
         <div className="w-full md:w-[49%] lg:w-[49%] xl:w-[49%] 2xl:w-[49%]">
           <Select
             ...
             variant="bordered"
             label={batchListLocale.select_a_period}
             labelPlacement="outside"
             placeholder={batchListLocale.select_a_period}
           ...
           >
             {initialDateTimeRangeOptionsBn.map((option) => (
               <SelectItem key={option.key}>
                 {
                   batchListLocale[
                     option.nameKey as keyof typeof batchListLocale
                   ]
                 }
               </SelectItem>
             ))}
         ...
         <div className="w-full md:w-[49%] lg:w-[49%] xl:w-[49%] 2xl:w-[49%]">
           <DateRangePicker
             ...
             label={batchListLocale.select_date_range}
             labelPlacement="outside"
         ...
         <div className="flex justify-between items-center mb-4">
           <div className="w-full md:w-[49%]">
             <Input
               size="md"
               aria-label="Search"
               label={batchListLocale.search}
               labelPlacement="outside"
               placeholder={batchListLocale.search_by_serial_no_outbound_or_inbound_address}
               variant="bordered"
         ...
         <div className="w-full grid grid-cols-1 ">
           <div className="flex justify-between items-center mx-2 my-2">
             <span className="text-default-400 text-small">
              {batchListLocale.total} : {lang==="bn"? e_to_b(filteredBallotRecords.length.toString()):filteredBallotRecords.length} {batchListLocale.ballot_records}
             </span>
 
             <div className="flex justify-center items-center">
               <label className="flex items-center text-default-400 text-small">
               {batchListLocale.rows_per_page} : {" "}
                 <select
           ...
           <div className="w-full overflow-x-auto shadow-md dark:bg-gray-900 rounded-md p-1">
             ...
             >
               <TableHeader columns={columns}>
                 {(column) => {
                   const tableHeader= batchListLocale[column.name as keyof typeof batchListLocale]||column.name;
                 ...
                 emptyContent={
                   <div className="text-center py-8">
                     <p className="ss:text-sm xxs:text-sm xs:text-base sm:text-base">
                       {batchListLocale.no_ballot_data_found}
                     </p>
                   </div>
                 }
               >
