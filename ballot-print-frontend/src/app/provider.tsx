...
const WeightMachineProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    // // Load printer ID from storage on mount
    // useEffect(() => {
    //     const storedPrinterId = getPrinterIdFromStorage();
    //     if (storedPrinterId) {
    //         setPrinterId(storedPrinterId);
    //     }
    // }, []);
    ...
    const checkConnection = async () => {
        ...
        try {
            const response = await fetch('http://localhost:30880/get-machine-id', { cache: 'no-store' });

            if (!response.ok) {
                setIsConnected(false);
                setPrinterId(null);
                // removePrinterIdFromStorage();
                setShowBanner(true); // Show banner to prompt download
                throw new Error('Failed to connect to weight machine');
            }

            const data: MachineIdResponse = await response.json();
            // console.log("Weight Machine ID Response:", data);
            if (data.Status === 'success') {
                setIsConnected(true);
                setPrinterId(data.Message);
                // setPrinterIdToStorage(data.Message); // Save to storage
                setShowBanner(true); // Show banner with printer ID
            } else {
                setIsConnected(false);
                setPrinterId(null);
                // removePrinterIdFromStorage(); // Remove from storage on error
                setShowBanner(true); // Show banner to prompt download
                throw new Error('Invalid response from weight machine');
            }
        } catch (err) {
            setIsConnected(false);
            setPrinterId(null);
    ...
    //     let defaultData: WeightDimResponse = {
    //         Tid: '',
    //         Length: '',
    //         Width: '',
    //         Height: '',
