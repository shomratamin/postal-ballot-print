...
export default async function fetchPostDms(post_id: string): Promise<FetchPostDMSResponseWrapper> {
    let cookieStore = await cookies()
    let url = `${process.env.NEXT_PUBLIC_DMS_API_URL}/dms/api/get-dms-article-web-by-user/?identifier=${post_id}`
    console.log("Fetching DMS article from URL:", url)
    try {
