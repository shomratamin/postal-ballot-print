...
import { PostStoreRequest, Post, PostStoreResponseWrapper, number_to_post_type, post_type_to_number } from "@/lib/store/post/types";
import { logout_user } from "@/lib/store/user/actions";
import { cookies } from "next/headers";

export const createPostDB = async (post: PostStoreRequest): Promise<PostStoreResponseWrapper> => {
    ...
    try {
        ...
        const response = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/api/bd-post/post-store`, {
            cache: "no-cache",
            method: 'POST',
            headers: {
