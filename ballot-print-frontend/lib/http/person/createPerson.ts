...
import { Person, PersonStoreRequest, PersonResponseWrapper, number_to_person_type, person_type_to_number, address_type_to_number, number_to_address_type } from "@/lib/store/person/types";
...
export default async function createPerson(person: PersonStoreRequest): Promise<PersonResponseWrapper> {
    let cookieStore = await cookies()
    console.log("got createPerson", person)
    ...
    formData.append('person_type', `${person_type_to_number(person.person_type)}`);


    console.log("url", `/api/bd-post/persons-store`, formData)

    try {

        const response = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/api/bd-post/persons-store`, {
            cache: "no-cache",
            method: 'POST',
            headers: {
        ...
        console.log("got person create response -> ", result)
        ...
        if (!response.ok) {
            console.log("response create person", response.body)
            return {
                data: null,
        ...
        let personStoreResponse: Person = {
            ...
            address_type: number_to_address_type(Number(result.data.address_type)),
        ...
        console.log("personStoreResponse", personStoreResponse)
        return {
            data: personStoreResponse,
